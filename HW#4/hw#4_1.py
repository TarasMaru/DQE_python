...
def antycheat(user_input: str, last_user_input: str, initial_len_of_input: int, user_agreed_letter: str) -> \
typing.Tuple[str, int]:
    ...
    # проверка чтобы согласована буква была использована
    has_not_agreed_letter: typing.List[bool] = [corrected_input[i] != user_agreed_letter for i in range(
        initial_len_of_input)]  # false - в паттерне игрока есть согласована ранее буква.

    agreed_letter_not_used: bool = all(
        has_not_agreed_letter)  # если существует хотя бы одно значение false в списке - паттерн валидный.
    while agreed_letter_not_used:
        print("Что-то тут не так.")
        print("Вы не использовали выбранной буквы.")
        print(f"А именно {user_agreed_letter}.")
        corrected_input = input("Давай, соберись. Попробуй снова   ").lower()

        has_not_agreed_letter: typing.List[bool] = [corrected_input[i] != user_agreed_letter for i in
...
def play_game():
    ...
    while is_playing:
        if was_correct:
            ...
            current_word, len_of_word = antycheat(current_word, last_word, len_of_word, user_agreed_letter)
